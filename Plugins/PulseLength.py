...
def calculate_pulse_length(data, A = [], dt = CONST.hene_fringe_fs, plot_results = True, print_results = True):
    ...
    if A == []:
        ...
        phase = 0
        A = [sigma, mu, offset, scale, frequency, phase]
    ...
    if y_env[150] < 0:
        y_env = -y_env
    max_env = numpy.amax(y_env)
    # select the part of the list
    l = numpy.where(y_env > max_env/2)[0]
